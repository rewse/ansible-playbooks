...
境界のずれは、駐車区間と車両状態が別のイベントから作られるために発生する。駐車区間の開始と終了は前後のドライブまたは充電プロセスから決まり、`online`、`asleep`、`offline`の区間は車両状態の取得と遷移から決まる。APIポーリングやStreaming APIの受信時刻には差があり、TeslaMateが次の状態を確認するまで直前の状態が続くため、状態区間が駐車区間より数秒から数分早く始まる、または遅く終わることがある。進行中の状態では`states.end_date`がNULLになる。

PR本文では、次のような時系列例を使ってこの挙動を説明する。
...
この例では9時間55分が重なるが、現行条件は`20:02 <= 20:00`を満たさないため、重複時間をすべて捨てる。境界をまたぐこと自体はデータ破損ではなく、非同期に記録されるイベントを突き合わせたときに起こりうる。
